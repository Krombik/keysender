...
//     Value toogleKey(const CallbackInfo &info)
//     {
//         Napi::Env env = info.Env();
//         if (info.Length() != 2)
//         {
...
//         ip.ki.wVk = 0;
//         ip.ki.dwExtraInfo = 0;
//         ip.ki.dwFlags = info[1].As<Boolean>() ? dwFlags : dwFlags | KEYEVENTF_KEYUP;
//         ip.ki.wScan = wScan;
...
//             return env.Undefined();
//         }
//         this->keyboardDelay = info[0].As<Number>().Int32Value();
//         return env.Undefined();
//     };
// };

class Hardware : public Napi::ObjectWrap<Hardware>
{
public:
    ...
    void toogleKey(const CallbackInfo &info)
    {
        Napi::Env env = info.Env();
        if (info.Length() > 3)
        ...
        ip.ki.dwExtraInfo = 0;
        ip.type = INPUT_KEYBOARD;
        ip.ki.dwFlags = info[1].As<Boolean>() ? flags(key) : flags(key) | KEYEVENTF_KEYUP;
        ip.ki.wScan = MapVirtualKeyA(key, MAPVK_VK_TO_VSC);
        SendInput(1, &ip, sizeof(INPUT));
        Sleep(info[2].As<Number>().Int32Value());
    }
...
class Virtual : public Napi::ObjectWrap<Virtual>
{
public:
    static Napi::Object Init(Napi::Env env, Napi::Object exports);
    Virtual(const Napi::CallbackInfo &info);
    UINT flags(UINT key)
    {
        UINT flags = KEYEVENTF_SCANCODE;
        if (find(begin(extendKeys), end(extendKeys), key) != end(extendKeys))
...
Napi::Object Hardware::Init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(
        env, "Hardware", {InstanceMethod("toogleKey", &Hardware::toogleKey)});
...
Napi::Object Virtual::Init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(
        env, "Virtual", {InstanceMethod("toogleKey", &Virtual::toogleKey)});
...
Virtual::Virtual(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Virtual>(info)
{
    Napi::Env env = info.Env();
}
...
// {
//     exports["toogleKey"] = Function::New(env, toogleKey);
//     return exports;
// }
