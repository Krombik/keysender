const { _textToImg, _getAllWindows, _getWindowChildren } = require('../../build/Release/key_sender.node');
...
const getFontName = path => {
    ...
    offset += 6;
    while (count--)
        if (data.getUint16(offset += 6) === 4 && [3, 0].some(item => item === data.getUint16(offset - 6))) {
...
module.exports = {
    textToImg(text, path, fontSize, options = {}) {
        if (options.color)
            options.color = rgbToBgr(options.color);
        ...
        return _textToImg(Buffer.from(text, "ucs2"), Buffer.from(path, "ucs2"), getFontName(path), fontSize, fullOptions)
    },
    getAllWindows() {
